...
export default {
    ...
    computed: {
        ...
        groupList() {
            ...
            if (this.totalPages <= this.pageGroup || this.totalPages <= 5) {
                // 初始化数据然后赋值
            ...
            // 判断这个位置与中间偏离的大小
            // 计算出中间值 ，校正左右位置的中间值    
            (this.currentPage < offset + 1) && (center = offset + 1); //校正起始位置,左侧

            //如果this.pageGroup是奇数，左右对称，否则，左右差1，要校正，如为8时就左4右3
            (this.currentPage > this.totalPages - offset) && (center = this.totalPages - offset + 1);
            //  校正结束位置，右侧           
            ...
            console.log(list);
            // 接着判断数组是否到达处于中间,添加...功能（也可以不加此功能，类似于百度分页）

            if (this.totalPages >= this.pageGroup) {
                // 校正添加“...”位置
                let addRigtPointPos = this.pageGroup % 2 == 0 ? this.totalPages - offset : this.totalPages - offset - 1;
                //  校正添加尾页码的位置
                let endPageNum = this.pageGroup % 2 == 0 ? this.totalPages - offset + 1 : this.totalPages - offset;

                ///////添加左侧...和首页码//////
                (this.currentPage > offset + 1) &&
                this.currentPage != offset + 2 && list.unshift({ txt: '...', val: list[0].val - 1 });
                //unshift左侧加
                (this.currentPage > offset + 1) && list.unshift({ txt: 1, val: 1 });
                /////添加右侧...和尾页码//////////////
                (this.currentPage < this.totalPages - offset) &&
                this.currentPage != addRigtPointPos && list.push({ txt: '...', val: list[list.length - 1].val + 1 });
                //修正
                // (this.currentPage == this.totalPages-offset-1) && list.push({txt:this.totalPages-1,val:this.totalPages-1});
                /////push右侧加
                (this.currentPage < endPageNum) && list.push({ txt: this.totalPages, val: this.totalPages });
            }
            //返回页码数组
            return list;
