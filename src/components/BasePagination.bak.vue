...
export default {
    ...
    computed: {
        ...
        groupList() {
            ...
            (this.currentPage<=offset+1) && (center =offset+1);//校正起始位置
             // console.log(center);
            (this.currentPage > this.totalPages - offset) && (center = this.totalPages - offset);//校正尾部位置
            // console.log(center);
            // 从当前位置减去偏移量再减去1就是数组索引，
            // 获取this.pagegroup长度数组
            temp = temp.splice(center - offset - 1, this.pageGroup); //获取可视页码部分（this.pageGroup个）
