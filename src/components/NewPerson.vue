    ...
    <div>
        <div class="dialog" v-if="show">
            <form action="" @submit.prevent>
                ...
                <div class="footer">
                    <button class="btn " @click="addData">确认操作</button>
    ...
    methods: {
        addData() {  //发送新增或编辑的数据
            if (!this.is_edit) { //新增数据
                this.$emit("pull", { name: this.name, age: this.age, sexy: this.sexy });

            } else {  //编辑数据
                this.$emit("pull", { id: this.id, name: this.name, age: this.age, sexy: this.sexy });
            }
        },
